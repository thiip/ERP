...
import { fullExpensesBidirectionalSync } from "@/lib/expenses-sync";
...
export async function POST(request: NextRequest) {
  try {
    ...
    for (const company of companies) {
      try {
        const result = await fullExpensesBidirectionalSync(company.id, month, year);
...
export async function GET() {
  return NextResponse.json({
    ...
    message: "Use POST to trigger expenses sync",
  });
}
