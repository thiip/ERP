...
import { getPayables, getPayablesSummary, markAsPaid, deleteInvoice } from "@/actions/financial";
...
export default function PayablesPage() {
  ...
  const [summary, setSummary] = useState<Summary>({ vencidos: 0, vencemHoje: 0, aVencer: 0, pagos: 0, total: 0 });
  ...
  async function loadData() {
    const [inv, sum] = await Promise.all([
      getPayables(month, year),
      getPayablesSummary(month, year),
    ]);
    setInvoices(inv as unknown as PayableInvoice[]);
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-5 gap-0 rounded-xl glass-card mb-4 overflow-hidden">
        ...
        <div className="px-4 py-3 text-center border-r border-foreground/[0.04]">
          <div className="text-xs text-foreground/50">Pagos (R$)</div>
          <div className="text-lg font-bold text-green-600">{fmt(summary.pagos)}</div>
        </div>
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-foreground/[0.08] bg-foreground/[0.02]">
              ...
              <th className="px-4 py-3 text-left text-xs font-medium text-foreground/50 uppercase">Resumo</th>
              <th className="px-4 py-3 text-right text-xs font-medium text-foreground/50 uppercase">Total (R$)</th>
              <th className="px-4 py-3 text-right text-xs font-medium text-foreground/50 uppercase">A pagar (R$)</th>
          ...
          <tbody className="divide-y divide-foreground/[0.04]">
            ...
            {filtered.map((inv) => {
              const remaining = Number(inv.totalValue) - Number(inv.paidValue || 0);
              return (
